...
import { ArrowLeft, Camera, Eye, Sun, Clock, MapPin, CheckCircle, Shield } from "lucide-react"
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
      ...
      <main className="flex-1 px-6 py-8">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <h1 className="text-3xl font-bold text-slate-900 mb-1">Ambil Foto</h1>
          <p className="text-slate-500">Pastikan wajah terlihat jelas</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <div className="bg-white rounded-2xl shadow-lg p-4 border border-slate-100 flex justify-between">
            <div className="flex items-center">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-400 to-emerald-500 rounded-xl flex items-center justify-center mr-3 shadow-md">
                <MapPin className="h-5 w-5 text-white" />
              </div>
              <span className="text-sm text-slate-700 font-medium">{location}</span>
            </div>
            <div className="flex items-center">
              <div className="w-10 h-10 bg-gradient-to-br from-violet-400 to-blue-500 rounded-xl flex items-center justify-center mr-3 shadow-md">
                <Clock className="h-5 w-5 text-white" />
              </div>
              <span className="text-sm text-slate-700 font-medium">{currentTime} WIB</span>
            </div>
          </div>
        </motion.div>
            ...
            {!capturedImage ? (
              <>
                <video
                  ...
                  playsInline
                  muted
                  className={`w-full h-full object-cover ${isCameraReady ? "opacity-100" : "opacity-50"}`}
                />
                    ...
                    >
                      ...
                      <svg className="absolute inset-0 w-full h-full rotate-90" viewBox="0 0 100 100">
                        ...
                        <motion.circle
                          ...
                          strokeDashoffset={301.59 - (301.59 * faceDetectionProgress) / 100}
                          strokeLinecap="round"
                        />
                      </svg>
                      ...
                      {faceDetected && (
                        <motion.div
                          ...
                          style={{
                            boxShadow: "0 0 0 2px rgba(16, 185, 129, 0.2), 0 0 0 4px rgba(16, 185, 129, 0.1)",
                          }}
                        ></motion.div>
                      )}
                    </motion.div>
          ...
          {isCameraReady && !capturedImage && (
            <div className="absolute -bottom-20 left-1/2 transform -translate-x-1/2">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`w-20 h-20 rounded-full bg-white shadow-xl flex items-center justify-center focus:outline-none transition-all ${
                  !faceDetected ? "opacity-70" : "hover:shadow-2xl"
                }`}
              >
                <div
                  className={`w-16 h-16 rounded-full ${
                    faceDetected ? "bg-gradient-to-r from-violet-600 to-blue-500" : "bg-slate-300"
          ...
          <div className="bg-white rounded-2xl shadow-lg p-5 border border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-start">
                <div className="w-10 h-10 bg-gradient-to-br from-violet-400 to-blue-500 rounded-xl flex items-center justify-center mr-3 flex-shrink-0 shadow-md">
                  <Eye className="h-5 w-5 text-white" />
                </div>
                <p className="text-sm text-slate-700">Wajah terlihat jelas dan tidak tertutup</p>
              </div>
              <div className="flex items-start">
                ...
                <p className="text-sm text-slate-700">Pencahayaan cukup (tidak gelap/silau)</p>
              </div>
            </div>
          </div>
        </motion.div>
